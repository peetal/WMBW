...
bias_info$hem <- bias_info$parcel_names %>%  #hem
  map(~ str_split(., "_") %>% 
        magrittr::extract2(., 1)) %>% 
  map(~ .[1]) %>% unlist()
bias_info$network <- bias_info$parcel_names %>% #network
  map(~ str_split(., "_") %>% 
        magrittr::extract2(., 1)) %>% 
  map(~ .[2]) %>% unlist()
bias_info$sub_net <- bias_info$parcel_names %>% #subnet
  map(~ str_split(., "_") %>% 
        magrittr::extract2(., 1)) %>% 
  map(~ .[3]) %>% unlist()


# from https://github.com/ThomasYeoLab/CBIG/blob/master/stable_projects/brain_parcellation/Yeo2011_fcMRI_clustering/1000subjects_reference/Yeo_JNeurophysiol11_SplitLabels/Yeo2011_7networks_N1000.split_components.glossary.csv  
# lpfc, vpfc vmpfc etc were all named into PFC
acronym <- c("Cing","Par","pCun","PFCl","OFC","PFCmp","PFCv","Temp",
             "pCunPCC","PFC","FEF","Post","PrCv","TempPole","FrOperIns","Med",
             "ParOper","TempOcc","PFCdPFCm","PrC","TempOccPar")
fullName <- c("Cingulate","Parietal","Precuneus","Frontal","OFC","Frontal","Frontal","Temporal",
              "Precuneus","Frontal","FEF","Posterior","Precentral","Temporal","FrOperIns","Medial",
              "Parietal","Temporal","Frontal","Precentral","Parietal")
# substitue acronyms with full names
for (i in 1:length(bias_info$sub_net)){ # i = 1
  if (bias_info$sub_net[i] %in% acronym){
    bias_info$sub_net[i] <- fullName[which(acronym == bias_info$sub_net[i])]
  }
  else{
    bias_info$sub_net[i] <- ""
  }
...
# group by both main network and sub-network
#bias_info %>% 
#  group_by(network, sub_net) %>%
#  summarise(Within_subject_effect_biased = sum(rank_diff >= 0),
#            Between_subject_effect_biased = sum(rank_diff < 0))

# group by only main network and hemisphere
bias_table <- 
  bias_info %>% 
  group_by(network) %>%
  summarise(Within_subject_effect_biased = sum(rank_diff >= 0),
...
# chi-square test for network
dorsoatten <- c(bias_table$Within_subject_effect_biased[which(bias_table$network == 'DorsAttn')], 
                bias_table$Between_subject_effect_biased[which(bias_table$network == 'DorsAttn')])
chisq.test(dorsoatten)

somMot <- c(bias_table$Within_subject_effect_biased[which(bias_table$network == 'SomMot')], 
            bias_table$Between_subject_effect_biased[which(bias_table$network == 'SomMot')])
chisq.test(somMot)
...
# group by only sub network, only care about frontal and parietal. 
bias_table2 <- 
bias_info %>% 
  filter(sub_net %in% c('Frontal', 'Parietal')) %>%
  group_by(sub_net, hem) %>%
  summarise(Within_subject_effect_biased = sum(rank_diff >= 0),
            Between_subject_effect_biased = sum(rank_diff < 0)) %>% 
  mutate(region = paste0(hem,"_",sub_net))
bias_table2 <- bias_table2[,c('region', 'Within_subject_effect_biased', 'Between_subject_effect_biased')]

rownames(bias_table2) <- NULL
colnames(bias_table2) <- c("Lobe", "Within-subject Variation \n Biased Region Count", "Between-subject Variation \n Biased Region Count")
bias_table2 %>%
  kbl(booktabs = T, align = "c") %>%
  kable_classic_2(full_width = F) %>%
  column_spec(., 2:3, width = "4cm") 
...
### Extra plotting
```{r, eval = F}
# all values were computed from notebook on github, here is just for plotting. 

data.frame("parcel" = c("Load-Activated (157)","Load-Deactivated (196)"),
                 "predAcc" = c(0.54, 0.44)) %>%
  ggplot(aes(x = parcel, y = predAcc, fill = parcel)) +
  ...
  theme(axis.title.x = element_text(size = 10),
        ...
        legend.title = element_blank()) +
  ylim(0, 0.6) + 
  ggsave(paste0("plots/suppmat/schaefer/load_act_vs_deact.svg"), width=3.5, height=2.5)
  

data.frame("effect_size" = c("01_03", "03_05", "05_07", "07_09", "09_11", "11_13",
                             "13_15", "01_03", "03_05", "05_07", "07_09"),
           "parcel_type" = c("Activated","Activated","Activated","Activated","Activated","Activated","Activated",
                             "Deactivated","Deactivated","Deactivated","Deactivated"),
           "pred_acc" = c(0.2739556885484732, 0.2895603686491316, 0.33847940221538164, 0.41085154212651737,
                          0.4577892038428485, 0.46989922184177557, 0.48181078671610855, 0.3698083284572665,
                          0.3026077479099265, 0.3384129818160498, 0.31189725523917694)) %>%
ggplot(aes(x = effect_size, y = pred_acc, color = parcel_type)) + 
  geom_point() + 
  geom_line(aes(group = parcel_type, color = parcel_type)) + 
  ylim(0, 0.6) + 
...
df <- import(here::here(paste0("data/schaefer_permutation/", "schaefer_feature_selection_permutation_WM_Task_2bk_Acc.csv")))
df$fs <- c(0.3409228365932808,
0.40347586684057385,
0.43857956671323683,
0.4452635481853114,
0.4926074335627305,
